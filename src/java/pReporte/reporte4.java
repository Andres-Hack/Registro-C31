 ...
 * @author andres
 */
@WebServlet(name = "reporte4", urlPatterns = {"/reporte4"})
public class reporte4 extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            ...
            String consulta2 = "SELECT        c.Codigo AS Cod_Subcomp, g.Partida,  SUM(a.Importe * a.Porcentaje) / (100*"+dolar+") AS Total_bs "
                + "FROM            Detalle AS a INNER JOIN "
                + "                          Actividades AS b ON a.c_Actividad = b.c_Actividad INNER JOIN "
                + "                          Subcomponentes AS c ON b.c_Subcomponente = c.c_Subcomponente INNER JOIN "
                + "                          Componentes AS d ON c.c_Componente = d.c_Componente INNER JOIN "
        ...
        Statement statementBID = null, statementCTR = null, statementPart=null;
        ResultSet rsBID = null, rsCTR = null, rsPart=null;
        try {
            for (int i = 0; i < 5; i++) {
                int sw1 = i + 1;
         ...
         try{
            ...
            Document documento = new Document();
             ...
             BaseColor color_actividad = WebColors.getRGBColor("#FAFEFF");
             ...
             PdfPCell celdaC = new PdfPCell(new Paragraph("PARTIDA", cabecera2));
             ...
             try {
                 ...
                 while (rsPart.next()) {
                    ...
                    String descrip = Consultas.nombrePartida(rsPart.getString("Partida"));
                     if (!BID.equals("0.0") || !CTR.equals("0.0")) {
                         tablesup.getDefaultCell().setBackgroundColor(color_actividad);
                         tablesup.addCell(new Paragraph("    " + rsPart.getString("Partida") + " " + descrip, actividad));
                         PdfPCell celdaP = new PdfPCell(new Paragraph(formatea.format(Double.parseDouble(BID)), montos));
                         celdaP.setHorizontalAlignment(Element.ALIGN_RIGHT);
                         celdaP.setBackgroundColor(BaseColor.WHITE);
                         tablesup.addCell(celdaP);
                         celdaP = new PdfPCell(new Paragraph(formatea.format(Double.parseDouble(CTR)), montos));
                         celdaP.setHorizontalAlignment(Element.ALIGN_RIGHT);
                         celdaP.setBackgroundColor(BaseColor.WHITE);
                         tablesup.addCell(celdaP);
                         celdaP = new PdfPCell(new Paragraph(formatea.format(Double.parseDouble(BID) + Double.parseDouble(CTR)), montos));
                         celdaP.setHorizontalAlignment(Element.ALIGN_RIGHT);
                         celdaP.setBackgroundColor(BaseColor.WHITE);
                         tablesup.addCell(celdaP);
                     }
